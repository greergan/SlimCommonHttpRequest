...
TEST_CASE("basic URL to Request", "[url to request]") {
...
TEST_CASE("basic URL to Request with / as path", "[url to request]") {
	auto url = URL("http://example.com/");
	Request request{url};
	auto& headers = request.headers();
	REQUIRE(headers.entries().size() == 2);
...
TEST_CASE("basic URL to Request with path", "[url to request]") {
